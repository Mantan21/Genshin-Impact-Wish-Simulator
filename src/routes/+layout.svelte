	<script>
		...
		import { authStore, checkAuth } from '$lib/store/authStore';
		...
		onMount(async () => {
			...
			try {
				...
				// Optional: Store user data in a global store if needed
			} catch (error) {
			...
			storageLocal.initEvent(); // for auto-export
			registerSW(); // Service Worker for faster load
			wakeLock(); // Prevent screen off while app is open
